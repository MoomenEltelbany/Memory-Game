...
// Selecting the restart button to restart the game
...
function creatingDiv() {
    for (let i = 0; i < userData.length; i++) {
        ...
        name.appendChild(
            document.createTextNode(`Hello: ${userData[i].userName}`)
        );
        ...
        myTriesSpan.appendChild(
            document.createTextNode(`${userData[i].numOfTries} tries`)
        );
        ...
        if (allShown.length === 2) {
            ...
            for (let i = 0; i < cards.length; i++) {
                ...
                if (matched === 4) {
                    let data = {
                        userName: yourName,
                        numOfTries: document.querySelector(".tries")
                            .innerHTML++,
                    };

                    for (let i = 1; i <= userData.length; i++) {
                        if (userData[i - 1].userName === data.userName) {
                            userData[i - 1].numOfTries = data.numOfTries;
                        }
                    }
...
for (let i = 0; i < userData.length; i++) {
    if (i === 0) {
        sortedArray.push(userData[i]);
    } else {
        if (userData[i].numOfTries > userData[i - 1].numOfTries) {
            sortedArray.push(userData[i].numOfTries);
        } else {
            sortedArray.shift(userData[i].numOfTries);
        }
    }
}
